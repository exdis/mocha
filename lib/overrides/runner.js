...
var reporter = require('mocha').reporters.Base;
...
Runner.prototype.log = function(msg, level) {
  level = level || 0;
  var pad = Array((level * 2) + 1).join(' ');
  console.log(pad + msg);
};

Runner.prototype.errLog = function(msg, level) {
  level = level || 0;
  var pad = Array((level * 2) + 3).join(' ');
  var prefix = reporter.symbols.err + ' ';
  console.log(reporter.color('fail', pad + prefix + msg));
};
